...
class linux_worker final : public host_worker {
public:
    linux_worker(worker_callback callback, void* context, detail::linux_worker_api api) noexcept
        : callback_(callback), context_(context), api_(api) {}

    ~linux_worker() override {
        if (!join()) std::terminate();
    }
    linux_worker(const linux_worker&) = delete;
    linux_worker& operator=(const linux_worker&) = delete;
    linux_worker(linux_worker&&) = delete;
    linux_worker& operator=(linux_worker&&) = delete;
...
private:
    static void run(void* raw) noexcept {
        auto& self = *static_cast<linux_worker*>(raw);
        try {
...
worker_result<std::unique_ptr<host_worker>> detail::start_linux_worker_with_api(
    ...
    try {
        auto owner = std::make_unique<linux_worker>(callback, context, api);
        owner->start();
...
worker_result<std::unique_ptr<host_worker>> start_linux_worker(
    worker_callback callback, void* context) noexcept {
    return detail::start_linux_worker_with_api(callback, context,
