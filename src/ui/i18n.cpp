...
// through I18n::Tr(key).  Currently only en-US is fully populated; other
// languages fall back to en-US for every key.  Adding a new language is a
// matter of filling in its table below.
//
...
// ---------------------------------------------------------------------------
// Populate all tables.  Only en-US gets real values; every other language
// copies the en-US pointer so Tr() naturally falls back.
// ---------------------------------------------------------------------------
void Init() {
    ...
    // -- All other languages: copy en-US pointers as fallback ------------
    for (int i = 1; i < static_cast<int>(Language::Count); ++i) {
        g_tables[i] = en;  // shallow copy — both maps point to same const char* values
    }
